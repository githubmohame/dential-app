class AdminsServicesController {
  ...
  async getAdminsServices(adminId) {
    new this.adminServiceRepos(this.adminService, this.next).getAdminsServices(
      id
    );
  }
  async addAdminService(adminId, serviceId, cost, star) {
    try {
      new this.adminServiceRepos(this.adminService, this.next).addAdminService(
        adminId,
        serviceId,
        cost,
        star,
  ...
  async deleteAdminsService(adminId, serviceId) {
    new this.adminServiceRepos(this.adminService, this.next).deleteAdminService(
      id
    );
  }
  async updateAdminService(adminId, map1, serviceId) {
    new this.adminServiceRepos(this.adminService, this.next).updateAdminService(
      adminId,
      map1,
      serviceId
    );
