class UserAdminAppoint{
    ...
    async getUserAppoint(adminId,adminServices){
     ...
     res=[...res,...this.userAdminAppoint.find({adminService:services[i]})]
     }
     return res;
    }
   async addUserAdminAppoint(userId,adminServiceId,date){
         let userAdminAppoint=new this.userAdminAppoint({user:userId,adminService:adminServiceId,date:date});
